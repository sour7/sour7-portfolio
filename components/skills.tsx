          ...
          >
            {row.map((skill, index) => (
              <motion.div key={skill.name} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Card className="group relative flex h-24 w-24 flex-col items-center justify-center gap-2 overflow-hidden bg-gray-800/50 p-4 transition-all duration-300 hover:bg-purple-600/20">
                  <motion.div
                  ...
                  />
                  <motion.i
                    className={`${skill.icon} text-3xl relative z-10`}
                    whileHover={{ rotate: 360 }}
                    transition={{ duration: 0.5 }}
                  />
                  <span className="text-center text-xs font-medium text-gray-300 transition-all duration-300 group-hover:text-white relative z-10">
                    {skill.name}
                  </span>
                </Card>
              </motion.div>
